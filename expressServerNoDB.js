...
const checkLogStatus = (req, res, next) => {
	if (req.session.userName != undefined) {
		let index = returnAccountIndex(loggedIn, req.session.userName);
		if (index != "NONE") {
			if (loggedIn[index].sessionID === req.sessionID) {
				next();
			} else {
				req.session.destroy();
				console.log("ALT LOG");
...
app.get('/userLogout', checkLogStatus, (req, res) => {
	loggedIn.splice(returnAccountIndex(loggedIn, req.session.userName), 1);
...
app.post('/login', (req, res) => {
	...
	for (let temp of allAccounts.values()) {
		if (temp.logName === checkUserLogName && temp.logPass === checkPass) {
			let index = returnAccountIndex(loggedIn, checkUserLogName);
			//ie, if this account already has someone logged in and it is not the person who just logged in
			//unpair the previous userName and session ID
			console.log(index);
			if (index != "NONE" && loggedIn[index].sessionID != req.sessionID) {
				loggedIn.splice(index, 1);
			}
			console.log("Correct Login!");
			console.log(temp.userName + " Is Logged In");
			let newLogged = JSON.parse(JSON.stringify(currLogged));
			newLogged.logName = checkUserLogName;
			newLogged.sessionID = req.sessionID;

			loggedIn.push(newLogged);
...
//Returns the index of a user account
function returnAccountIndex(group, accName) {
	let i = 0;
